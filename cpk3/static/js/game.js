...
function add_code(){
	var title;var mode;var codekey;codeHTML="";
	...
	for (i=0;i<arrGameCodes.length;i++ )
	{
		...
		if(currentcode==""){currentcode=arrGameCodes[0];currentmode=arrCodes[currentcode].mode;}
		title="";mode="";codekey="";
...
function add_plays(item,modes,next){
  ...
  if(arrCodes[o]['pid']==item){
	...
	var playid,CodeTile,ShowTile,Rebates;var lastCode="";var playHTML="";var codeshow="";var itemHTML="";var checkon="";var endcodediv="";var ids=0;
	...
	for (var ii in plays)
	{
        ...
        if(wanfa.indexOf(playid)>-1){
            ...
            if(plays[ii].Rebates){Rebates=plays[ii].Rebates;}else{Rebates="Normal";}
            if(len-1<0){
                checkon="checked";
...
//	DialogAlert(shownum);
	...
	*/
	...
	if(rebates!=""){selplay.retype=rebates;}
	//playlist:{playid:'',shownum:'',minnum:'',maxnum:'',show_key:'',show_other:'',max_select:'',min_select:''};

    //DialogAlert(plays.playid);
	var game_cate=G('game_cate').innerHTML;
//DialogAlert(shownum);
if(play_item.indexOf('QP')>-1 || play_item=='LH' || play_item=='HZ-k3' || play_item.indexOf('QWX2')>-1  )wanfa_cate='qw';
else wanfa_cate='ct';
	...
	if(wanfa_cate=='qw'){
        ...
        G("lt_sel_insert").onclick=function(){qwinsertline()}
//console.log(rebates);
        ...
        G("lt_sel_insert").onclick=function(){selinsertline()}
        if((shownum=="0") && (show_other=="0") && (show_key=="0")){
            ...
            if(play_item=='TJZH'){
                ...
                G("lt_sel_insert").onclick=function(){boxinsertline()}
            }
            else {
                //单式
...
var str='';
        ...
        if(modes=="jiao"){prize=prize/10;}
        if(modes=="fen"){prize=prize/100;}
        if(modes=="li"){prize=prize/1000;}
...
if(selplay.CurModeType=="auto"){
	...
	if(G('SelectAutoMode')){G('SelectAutoMode').style.display="";}
	resetAutoMode();
}else{
...
function count_select_arr(){
	selists=new Array();
	var shownum=0;var minnum=0;var maxnum=0;var Objs;var n=0;var newlines=0;
	...
	if(playlist.show_other.indexOf("|")>0 || playlist.show_other.length>1){
		...
		for (i=0;i<arrs.length;i++){
			...
			selists[i]=new Array();n=0;
			for (j=minnum;j<=maxnum;j++){
				if(G("select_"+i+"_"+j)){
					Objs=G("select_"+i+"_"+j);
					if(Objs.className.indexOf("cur")>0){selists[i][n]=Objs.innerHTML;n+=1;}
				}
			}
		}
	}
...
function Count_Money(){
    ...
    if(wanfa_cate=='qw'){
		...
		if (Is_str_type(lt_sel_nums,'1')){is_ok="yes"}else{return false;}
  ...
  //  if (Is_str_type(lt_sel_nums,'1')){is_ok="yes"}else{times=1;}
	...
	if(nums>=1){   //注数大于0时，才能有值
		...
		if(modes=="yuan"){moneys=nums*times*2;}
		if(modes=="jiao"){moneys=nums*times*2/10;prize=prize/10;}
		if(modes=="fen"){moneys=nums*times*2/100;prize=prize/100;}
		if(modes=="li"){moneys=nums*times*2/1000;prize=prize/1000;}
...
if(moneys>0) {
	if(modes=="yuan"){moneys=moneys.toFixed(0);prize=parseFloat(prize).toFixed(0);}
	if(modes=="jiao"){moneys=moneys.toFixed(1);prize=parseFloat(prize).toFixed(1);}
	if(modes=="fen"){moneys=moneys.toFixed(2);prize=parseFloat(prize).toFixed(2);}
	if(modes=="li"){moneys=moneys.toFixed(3);prize=parseFloat(prize).toFixed(3);}
}
...
function IsMax_Times(vthis){
	var times=1;var $is_ok_l="no";
	if(vthis.id=="times"){ times = parseInt(getselectItem(vthis),10);$is_ok_l="yes";}
	if(vthis.id=="lt_sel_times"){ times = parseInt(vthis.value,10);$is_ok_l="yes";}
   ...
   //console.log(ajax_last_timer);
        ...
        ajaxobj.callback=function(xmlobj){
            ...
            lastlotTimer=setTimeout(function(){
                if(document.getElementById('last_issues').innerHTML!=selplay.pre_period && selplay.isbuy==1) {
                    Ajax_last_lotnum();
                }
                else {
...
function Show_last_lotnum(response){
	...
	if(response.length-5<0){return false;}
	var lists=response.split("^");

	try
	{

		var last_issue=document.getElementById('last_issues').innerHTML;
		var current_issue=document.getElementById('current_issue').innerHTML;
	...
	if(last_issue!=lists[1] && current_issue!=lists[1]){
...
function period_loading() {
    ...
    if(selplay.isbuy==0 && selplay.stoptime>0){

        process_loading(parseInt(selplay.stoptime),parseInt(selplay.lastsecond));
...
//DialogAlert(code);
    ...
    if(selplay.pre_period!=issue || selplay.isbuy!=1){
...
function show_code11(aTmp){
    ...
    if(gametype=='11x5'){
        ...
        if(gametype=='k3'){
            var sum=0;var num2=0;
            var sum=parseInt(code[0])+parseInt(code[1])+parseInt(code[2]);
            if(sum<=9) str="<span>小</span>";else str="<span>大</span>";
...
function periodlists(nowdate_s,nextdate_s,lastdate_s,nowtime_s){
	...
	var thispri="";var thistime="";var nowpri="";var fl_num_a=0;var fl_num_b=0;var nowtime="";var do_key="ss";
    var lotarr_a=new Array();var lottime_a=new Array();
	var lotarr_b=new Array();var lottime_b=new Array();
    if(gamekey=="3D" || gamekey=="P5(P3)"|| gamekey=="PL3" || gamekey=='LF3d'){do_key="dp";}
    if(gamekey=="LJSSC" || gamekey.indexOf("KL8")>0){do_key="fl";}
	var is_next_date="no";//DialogAlert(nowtime+"|"+endtime)
    if(parseInt(offset)-1==0){
		if(nowtimes-endtime>0){is_next_date="yes";nowdate=nextdate;nowdate_s=nextdate_s;}
	}
	for (var i in arrTimes){
		thispri=parseInt(arrTimes[i].period,10);
		if(thispri-offset>=0){
			if(do_key=="ss"){nowpri=nowdate+arrTimes[i].period;lotarr_a.push(nowpri);}
			if(do_key=="fl"){nowpri=parseInt(lotpriod,10)+fl_num_a;lotarr_a.push(nowpri);fl_num_a+=1;}
			nowtime=nowdate_s+" "+arrTimes[i].ends;lottime_a.push(nowtime);
		}
		if(offset-thispri>0){
			if(do_key=="ss"){nowpri=nextdate+arrTimes[i].period;lotarr_b.push(nowpri);}
			if(do_key=="fl"){nowpri=parseInt(lotpriod,10)+fl_num_b;lotarr_b.push(nowpri);fl_num_b+=1;}
			nowtime=nextdate_s+" "+arrTimes[i].ends;lottime_b.push(nowtime);
		}

	}
	if(do_key=="dp"){
		nowpri=selplay.lotpriod;lotarr_a.push(nowpri);nowtime=selplay.lotendtime;lottime_a.push(nowtime);
	}
    lotarr_a.sort();lottime_a.sort();lotarr_b.sort();lottime_b.sort();
	lotarr_a.concat(lotarr_b);
	lottime_a.concat(lottime_b);
	//打印起始期
	peroarr=[];perotimearr=[];
	var select_lot = document.getElementById("list_lot_num");
	select_lot.options.length = 0; var option1;var f=0;
...
function selinsertline(){
	...
	if((selplay.modes=="jiao") || (selplay.modes=="角")){
		...
		if(MinModeJiao){
			...
			{
				if(parseInt(MinModeJiao,10)-parseInt(times,10)>0){DialogAlert("角模式下注单倍数至少为"+MinModeJiao+"倍！");return false;}
			}
		}
	}
	if((selplay.modes=="fen") || (selplay.modes=="分")){
		...
		if(MinModeFen){
			...
			{
				if(parseInt(MinModeFen,10)-parseInt(times,10)>0){DialogAlert("分模式下注单倍数至少为"+MinModeFen+"倍！");return false;}
			}
		}
	}
	...
	selists=[];clear_sel();
	G('lt_cf_count').innerHTML=parseInt(G('lt_cf_count').innerHTML,10)+1;  set_cf_count_display();
...
//	}
//
	var flags="yes";var flaginfor="no";
...
var wei_item='';
	...
	//判断字段是否为空
	if(nums=="0" || typeof(nums)=="undefined"){flags="no";flaginfor="nums";}
	if(gamekey=="" || typeof(gamekey)=="undefined"){flags="no";flaginfor="gamekey";}
	if(playid=="" || typeof(playid)=="undefined"){flags="no";flaginfor="playid";}
	if(modes=="" || typeof(modes)=="undefined"){flags="no";flaginfor="modes";}
	if(CurMode=="" || typeof(CurMode)=="undefined"){flags="no";flaginfor="CurMode";}
	if(CurModeType=="" || typeof(CurModeType)=="undefined"){flags="no";flaginfor="CurModeType";}
	if(times=="" || typeof(times)=="undefined"){flags="no";flaginfor="times";}
	if(lotpriod=="" || typeof(lotpriod)=="undefined"){flags="no";flaginfor="lotpriod";}
	if(lines=="" || typeof(lines)=="undefined"){flags="no";flaginfor="lines";}
	if(showcode=="" || typeof(showcode)=="undefined"){flags="no";flaginfor="showcode";}
	...
	G('lt_cf_money').innerHTML=comoney;	hm_check();
  fly_cart();
}
...
function getBuyInfor(){
	...
	var modes="";var codels="";var numbers="";
	...
	for (a=0;a<selArr.length;a++){
		...
		//DialogAlert(arrs[12])
		if(arrs[12].length-25>0){numbers=arrs[12].substr(0,23)+"..";}else{numbers=arrs[12];}
	...
	if(mobile==1) {
        ...
        var retuenHTML="<div style='height:30px;line-height:40px;text-align:left;clear: both;font-size:16px;'>"+gamename+":"+qihao+"期</div>";

		retuenHTML+="<div style='height:30px;line-height:30px;text-align:left;font-size:16px;padding-top: 5px;'>投注金额:<span style='color:#ff0000'>"+moneys+"元</span></div>";
				...
				try
{
		...
		var temp22=temp11[13].split(",");
                    ...
                    if(temp22[jj]==1){ wei_str+=weiarr[jj];
...
function clearsels(){
	...
	buylist='';
    clearTask();	hm_check();  set_cf_count_display();
}
//追号==========================================
...
function click_zuihao(type) {
	...
	if((is_zui==false && type==0) || (is_zui==true && type==1)){
        ...
        if(G('lt_cf_count').innerHTML=="0"){ document.getElementById('zuihao_click').checked=false;  G('lt_trace_stop').checked=true;return false;}
        else
...
function addgametask1(ss,div){
	...
	if(ss==true){
		if(G('lt_cf_count').innerHTML=="0"){DialogAlert("请添加投注号码再发起合买！");
...
function clearTask(){
	clearTaskSel();
	seltask.nums=0;
	if(G('lt_trace_if_button')){G('lt_trace_if_button').checked=false;}
G('lt_trace_stop').checked=true;
	if(G('lt_trace_box1')){G('lt_trace_box1').style.display="none";}
//	G('lt_trace_box2').style.display="none";
...
function Ajax_get_mmssc(){
	...
	ajaxobj.callback=function(xmlobj){var response = xmlobj.responseText;};ajaxobj.send();
...
//	G("history_info_1").innerHTML="<tr><td colspan='3' align='center'>正在刷新...</td></tr>";
	...
	ajaxobj.callback=function(xmlobj){var response = xmlobj.responseText;Show_get_buy(response);};ajaxobj.send();
...
//DialogAlert(skey);
	if(skey.length<10){G("history_info_1").innerHTML="<tr><td colspan='12' align='center'><i class='icon-bell-2 fa-4x' ></i><br>该彩种最近7天没有投注记录</td></tr>";return false;}
	var code=skey.split("#");var codenum=code.length;var lists;var lastcate=new Array();
	var titles="";var innerHTML="";

	var length=code.length;
	//if(length>3) length=3;
	for (i=0;i<length;i++ ){

		lists="";lists=code[i].split("^");
		if(lists[0].length-15>0){titles=lists[0].substr(0,15)+"..";}else{titles=lists[0];}
		var zhao;
		if(lists[10]=="yes"){
      ...
      //  innerHTML+="<td><a title='"+lists[0]+"' onclick=\"javascript:DialogResetWindow('查看投注单','"+open_url+"','700','460')\" style='cursor:pointer;' title='查看注单详情' class='blue'>"+titles+"</a></td>"
       ...
       // innerHTML+="<td "+onclick+">"+lists[7]+"</td>"  ;
        ...
        if(lists[4].length-15>0){var con=lists[4].substr(0,15)+"..";}else{var con=lists[4];}
...
//	G("history_info_2").innerHTML="<tr><td colspan='3' align='center'>正在刷新...</td></tr>";
	...
	ajaxobj.url=$("#do_url").val()+"?mod=ajax&code=get&list=data&flag=yes";
	ajaxobj.callback=function(xmlobj){var response = xmlobj.responseText;Show_get_buy2(response);};ajaxobj.send();

}

function Show_get_buy2(skey){
	//console.log(skey);
	if(skey.length<10){G("history_info_2").innerHTML="<tr><td colspan='12'  align='center'><i class='icon-bell-2 fa-4x' ></i><br>该彩种最近7天没有追号记录</td></tr>";return false;}
	var code=skey.split("#");var codenum=code.length;var lists;var lastcate=new Array();
	var titles="";var innerHTML="";
...
function Ajax_get_buy3(){
	...
	ajaxobj.url=$("#do_url").val()+"?mod=ajax&code=get&list=data&flag=yes";
	ajaxobj.callback=function(xmlobj){var response = xmlobj.responseText;Show_get_buy3(response);};ajaxobj.send();

}

function Show_get_buy3(skey){

    if(skey.length<2){G("history_info_3").innerHTML="<tr><td colspan='12'  align='center'><i class='icon-bell-2 fa-4x' ></i><br>您最近7天没有合买记录</td></tr>";return false;}
    var code=    JSON.parse( skey );

    var length=code.length;

var codenum=code.length;var lists;var lastcate=new Array();
	var titles="";var innerHTML="";
...
CheckTxt11(div,id);CheckTxt(div);Count_Money();
...
function getBuyInfor1(){
	...
	for (a=0;a<selArr.length;a++){
		...
		if(codels.length-25>0){numbers=codels.substr(0,23)+"..";}else{numbers=codels;}
...
var nums=0;
    ...
    var showcode="["+st+"]"+click_str;
			...
			if(modes=="yuan"){money=nums*times*2;var mode='元'}
			if(modes=="jiao"){money=nums*times*2/10;var mode='角';}
			if(modes=="fen"){money=nums*times*2/100;var mode='分';}
			if(modes=="li"){money=nums*times*2/1000;var mode='厘';}
...
function CZH(){
	...
	if(TouZhuMoneys[i].value>0) {temp=1;break;}
...
return  false ;
	...
	if(confirm('确定要投注吗? ')==false){return false;}
	else{
  ...
  message_top();
	...
	//	DialogAlert(ajaxobj.content);return false;
		...
		ajaxobj.callback=function(xmlobj){
			...
			document.getElementById("lt_cf_count").innerHTML="0";  set_cf_count_display();
			document.getElementById("lt_cf_money").innerHTML="0";
			document.getElementById("lt_cf_nums").innerHTML ="0";
			document.getElementById("lt_prize_money").innerHTML ="0";
			document.getElementById("lt_prize_money11").innerHTML ="0";
...
function CheckTxt(obj){
   ...
   if(objv.length>0){
	   ...
	   if(parseInt(objv)>parseInt(xb)){
		   DialogAlert("超过最大限倍");$(obj).val(objv.substring(0,objv.length-1));
		}
		//写入选号标识
...
function wait_time12(){
	...
	setInterval(function(){wait_time1();},1000);
}
function wait_time1(){
