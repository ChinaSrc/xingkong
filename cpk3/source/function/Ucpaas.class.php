...
class Ucpaas
{
    ...
    private function getResult($url, $body = null, $method,$accessToken)
    {
        $data = $this->connection($url,$body,$method,$accessToken);
    ...
    private function connection($url, $body,$method,$accessToken)
    {
    ...
    public function SendSms1($appid,$templateid,$param=null,$mobile,$uid){
        // $url = self::BaseUrl . 'sendsms';


        $operation = "";
        $time = date('YmdHms');
        $SigParameter = $this->accountSid.$this->token.$time;
        // $SigParameter = $appid.$this->token.$time;
        $SigParameter=  strtoupper(md5($SigParameter));


        $Authorization = base64_encode($this->accountSid.":".$time);
        // var_dump($Authorization);exit();

        $url = "https://api.ucpaas.com/2014-06-30/Accounts/".$this->accountSid."/Messages/templateSMS?sig={$SigParameter}";
            // 'token'=>$this->token,
        $body_json = array(
            // 'sid'=>$this->accountSid,
            'appId'=>$appid,
        ...
        $body_json = array('templateSMS' =>$body_json );

        $body = json_encode($body_json);
        $data = $this->getResult($url, $body,'post',$Authorization);
        return $data;
    }
