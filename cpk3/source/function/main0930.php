...
if($money<0  and $type!='active') $money=-$money;
...
$rebate1=unserialize($arr[$i]['rebates']);
	...
	$sql="insert into user_fandian_log(uid,fromid,money,time,buyid) values('{$arr[$i]['userid']}','{$buy['userid']}','{$money2}','".time()."','{$buy[id]}')";
	$db->query($sql);
...
function  show_buystatus($buyinfo,$type=0){
 if($buyinfo[isprize]==""){$status="<font color='green'>未开奖</font>";}
if($buyinfo[isprize]=="is_yes"){$status="<font color='red'>已中奖</font>";}

if($buyinfo[isprize]=="is_no"){$status="<font color='blue'>未中奖</font>";
 if($type==1){$status="<font color='#fff'>未中奖</font>";}
 }
if($buyinfo[status]=="9"){$status="<font color='green'>已撤单</font>";}
...
.bigIcon{ margin-left:35px;width: 48px; height: 48px; background-image: url(/template/default/2017/images/icons.png); background-repeat: no-repeat; background-position: 0px -48px;}
    </style>
    <div  style='height:180px;max-width:400px;width:<?php if ($_GET['mobile']==1) echo '95%';else echo "400px"; ?>;margin:100px auto;padding-top: 2px; border-radius: 5px;    border:1px solid #d4d2cc;'>
            ...
            <table  height="200" border="0" cellpadding="0" cellspacing="0" style="width:100%;">
              ...
              <tr>
                ...
                <td align="center" valign="top" style="padding:20px;font-size:15px;color:#000;font-weight:400;line-height:30px;">
            <?php
...
.bigIcon{ margin-left:35px;width: 48px; height: 48px; background-image: url(/template/default/2017/images/icons.png); background-repeat: no-repeat; background-position: 0px -48px;}
 ...
 .btnArea{border-top: solid 1px #eef0f1;padding:10px auto;margin-bottom:10px;}
 .btnGroup{width:100%;text-align:center;}
.btnGroup .sgBtn{margin-top: 14px; margin-right: 10px;}
 .sgBtn{display: inline-block; cursor: pointer; width: 95px; height: 35px; line-height: 35px; text-align: center; color: #FFFFFF; border-radius: 5px;}
 .sgBtn.ok{background-color: #f37030; color: #FFFFFF;}
.sgBtn.cancel{background-color: #546a79; color: #FFFFFF;}
 #BgDiv{background-color:#000000; position:absolute; z-index:1; left:0; top:0;  width:100%; height:100%;opacity:0.7;filter: alpha(opacity=40);-moz-opacity: 0.7;}
    </style>
...
<div id="BgDiv" ></div>
    ...
    <div  style='height:200px;position:absolute;top:200px;z-index:100;background-color:#fff;<?php if ($_GET['mobile']==1) echo 'width:95%;left:2%;';else echo "width:400px;left:40%;"; ?>; border-radius: 5px;  z-index:22;  border: 5px solid #d4d2cc;'>
            ...
            <table  height="190" border="0" cellpadding="0" cellspacing="0" style="width:100%;">
              ...
              <tr>
                <td height="32" colspan="2" style="border-bottom: solid 1px #eef0f1;background-color:#f7f7f7;line-height:32px;font-size:18px;color:#222;font-weight:700;padding-left:10px;">温馨提示</td>
              </tr>
              <tr>
              ...
              <div class='bigIcon' style='background-position:<?php echo $pos;?>;'></div>
                ...
                <td align="left" valign="middle" style="padding:20px;font-size:15px;color:#000;font-weight:400;line-height:30px;">
...
<div class="btnGroup"><a class="sgBtn ok" onclick='window.close();'>确定</a><a class="sgBtn cancel" id='cancel'  onclick='window.close();'>取消(5)</a></div>
                </td>
            </tr>
...
setInterval(function(){note_change();},1000);
...
function check_server(){
    ...
    $install_time=$con_system['install_time'];
    ...
    if(strpos($install_ip,$server_ip)===false){
        ...
        $install_time++;
        update_sys('install_time',$install_time);
    }

    if($install_time>=30) exit();
...
include_once "source/config/play/lot_time_".$gamekey.".php";
    ...
    foreach ($time_arrs as $key=>$value){
        $num++;
        if(($nowtime>=$value['begin'] and $nowtime<$value['lot']) or  ($key==24 and $gamekey=='CQSSC' and $nowtime>=$time_arrs[$key-1]['lot'] and $nowtime<$value['begin'] )){
            $period=date('Ymd').$key;
          $lotNum=$key;
            $stoptime=strtotime($value['lot'])-strtotime($value['end']);
            //封单
            if($nowtime>$value['end']) {
                $isbuy=0;
                $lastsecond=strtotime($value['lot'])-strtotime($nowtime);

            }
            else{

                $isbuy=1;
                $lastsecond=strtotime($value['end'])-strtotime($nowtime);
            }
            if($num>1) $pre_period=$period-1;
            else{
                $arrs1=array_reverse($time_arrs);
               foreach ($arrs1 as $key1=>$value1){
                   $pre_period=date('Ymd',time()-24*3600).$key1;
                   break;
               }
...
$first=key($time_arrs);
    if($nowtime<$time_arrs[$first]['begin']){

        $period=date('Ymd',time()).$first;
        $lastsecond=strtotime(date('Ymd',time()).' '.$time_arrs[$first]['end'])-time();
        $num=0;
        $isbuy=1;
        $return['begin']=$time_arrs[$first]['begin'];
        $return['end']=$time_arrs[$first]['end'];
        $return['lot']=$time_arrs[$first]['lot'];
        $pre_period=date('Ymd',time()-24*3600).$key;
    }
...
$last=$key;
    ...
    if($num==$sum and $nowtime>$time_arrs[$last]['lot']){
        foreach ($time_arrs as $key=>$value){

                $period=date('Ymd',time()+24*3600).$key;
                $lastsecond=strtotime(date('Ymd',time()+24*3600).' '.$value['end'])-time();
        ...
        $pre_period=date('Ymd',time()).$last;

    }
    if($gamekey=='BJK3'){
        $start=strtotime('2018-07-25 09:01:00');
        ...
        if(time()>strtotime(date('Y-m-d').' 23:51:00')){
            ...
            $sec=floor((time()-strtotime(date('Y-m-d').' 09:01:00'))/600);
            if($sec<0) $sec=0;
...
function lottery_list($gamekey,$num=10){
   ...
   if($game['self']==1 or $period['isbuy']==1){

       $sql="select * from game_lottery where playKey='{$gamekey}' and period<'{$period['period']}' order by period desc limit 0,{$num}";
   }
   else
       $sql="select * from game_lottery where playKey='{$gamekey}' and period<='{$period['period']}' order by period desc limit 0,{$num}";
...
$str=' <div class="userinfo">
                     <div  class="card">
                        <div  class="cardLeft">
                            <img  src="'.avatar($user['userid']).'" alt="" width="80" height="80">
                            <h6 >';
                            if($user['nickname']) $str.= $user['nickname'] ;else $str.= "未设置昵称";
...
$str.='</h6>
    ...
    $str.='</div>
           ...
           function number_show($str,$num=2){
            ...
            if(!$str) return 0;
            if(strpos($str,'.')) return number_format($str,$num,'.','');
            else return $str;
