...
?>
    ...
    <link href='template/default/2017/css/mycenter.css' rel="stylesheet" type="text/css" />
<style>
    ul {
        list-style: none;
    }
...
if($active=='user_rebates'){
    ...
    <div class="rebateList" style="width: 360px;margin: 10px auto;padding: 0px;border-width: 0px;">
        <?php

        foreach ($rebates as $key=>$value){
            ?>
            <ul class="InviteReb"><li style="width: 80px;padding-left: 60px;"><?php echo $arr_game_code[$key]; ?></li>
                <li  ><input type="text" name="<?php echo $key;?>" value="<?php echo $value;?>" disabled="disabled" class="userInput mgl20">&nbsp;
          </li>
            </ul>
...
if($active=='show_playuserlist'){
  ...
  $list=  $db->fetch_all("select * from game_buylist where is_succeed='yes' {$str} and pri_money>0 order by prize_time desc  limit 0 ,{$num}");
...
if(count($list)>0){
  ...
  ?>
        ...
        <li  <?php if($mobile==1){ ?> onclick="location.href='index_player.html?id=<?php echo $value['userid'];?>';" <?php } ?> >
            <img  src="<?php echo avatar($value['userid'])?>" class="avatar"/>
            <p ><?php if($user['nickname']) echo $user['nickname'] ;else echo GBsub_str($user['username'],0,2).'*****'.GBsub_str($user['username'],strlen($user['username'])-1,1);?> <?php echo $game['fullname']?><br  />喜中<span >&yen;<?php echo $value['pri_money']?></span></p>
    ...
    if(count($list)>0) {
        ...
        foreach ($list as $value) {
            if ($value['sum1'] > 0) {
                ...
                <li <?php if ($mobile == 1) { ?> onclick="location.href='index_player.html?id=<?php echo $value['userid']; ?>';" <?php } ?> >
                    <div class="info1"><img src="<?php echo avatar($value['userid']) ?>" class="avatar"/>
                        <p>
                            账号昵称：<?php if ($user['nickname']) echo $user['nickname']; else echo GBsub_str($user['username'],0,2) . '*****' . GBsub_str($user['username'], strlen($user['username']) - 1, 1); ?>
                            <br/> 昨日奖金：<i>&yen;<?php echo number_format($value['sum1'], 2, '.', ''); ?></i></p>
                    </div>
                    <div class="info2">
