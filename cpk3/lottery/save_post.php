...
if ($active == "game_type") {
    ...
    for ($i = 0; $i < count($modes); $i++) {
        if ($modeslist == "") {
            ...
            $modeslist = $modeslist . "|" . $modes[$i];
        }
        if ($first_cate == $modes[$i]) {
            $this_code_num = $i;
            $is_inarray = "yes";
        }
    }
    foreach ($modes as $value) {
        ...
        if ($list1) {
            foreach ($list1 as $value1) {
                if (!in_array($value1['ckey'], $modeslist_arr)) {
                    $modeslist = $modeslist . "|" . $value1['ckey'];
                }
            }
        }
    }
...
if ($active == 'admin') {
    ...
    if ($_POST['password'] and $id > 0) {
        md5_pwd($id, $_POST['password']);
    }
    if ($_POST['status'] != '1') {
        $db->query("delete from login_error where username='{$_POST['username']}'");
    }
    if ($_GET['from'] == 'admin') {
...
if ($active == "user") {
    ...
    if ($_POST['pname']) {
        ...
        if ($high) {
            if ($id == $high['userid']) {
                ...
                //下级列表
                $childarr = child_node($id);
                $childarr = explode(",",$childarr);
                if (in_array($high['userid'], $childarr)) {
                    echo "<div style='font-size:14px;text-align:center;background:#FFFFFF;'><font style='color:#ff0000;'>上级代理不能填写自己的下级</font></div>";
                    echo "<script>setTimeout(\"history.back()\",1000)</script>";
                    exit;                    
                }
            }
    ...
    if ($id > 0) {
        $strSql = "update user set sex='{$sex}',tranfer='{$_POST['tranfer']}',status='{$_POST['status']}',modes='{$modes}',higherid='{$higherid}',isproxy='{$isproxy}',proxynum='{$proxynum}',mark='{$_POST['mark']}',fenhong='{$_POST['fenhong']}',ps_pre1='{$_POST['ps_pre1']}',user_tab='{$_POST['user_tab']}',ps_pre2='{$_POST['ps_pre2']}' {$pass} where userid='{$id}'";
...
if ($active == "CodeListShow") {
    ...
    if ($bodyHTML == "") {
        $bodyHTML = "<tr height=40 bgcolor='#FFFFFF'><td align=center colspan=5>未找到</td></tr>";
    }
    $bottomHTML = "</table>";
...
if ($active == "t_delete") {
    ...
    if (strpos($table, "?") !== false) {
        ...
        $table = $t[0];
        $tt = explode("=", $t[1]);
        $str = " and `{$tt[0]}`='{$tt[1]}' and status!='1' ";
    } else {
        $str = '';
    }
    if ($table == 'game_lottery') {
...
if ($active == "SetTimes") {
    ...
    $flags = "yes";
    $add_flag = "yes";
    if ($ForGames == "" or $ForUsers == "" or $ForRange == "" or $number == "") {
        $flags = "no";
        exit;
    }
    if ($ForUsers != 'all') {
...
if ($active == "GetSetTimes") {
    ...
    if ($num6) {
        while ($rows6 = mysql_fetch_array($result6)) {
            $ForUsers = $rows6[ForUsers];
            $ForPlays = $rows6[ForPlays];
            if ($ForUsers != "all") {
...
if ($active == "ajaxSaveInfor") {
    ...
    if ($dbname == "" or $id == "" or $values == "" or $item == "") {
        ...
        if ($dbname == "user" or $dbname == "user_bank") {
            $cid = "userid";
        } else {
            $cid = "id";
        }
...
if ($active == 'lottery_add') {
    ...
    if ($error) {
        ...
        $str = lottery_add($_POST['playKey'], $_POST['period'], $number);
        if ($str == true) {
            if ($_POST['id']) {
                add_adminlog("手动修改开奖数据");
                if ($_POST['prize_back'] == 1 and $old['Number'] != $number) {
                    $query = $db->query("select * from game_buylist where playkey='{$_POST['playKey']}' and period='{$_POST['period']}' order by id asc");
                    while ($row = mysql_fetch_array($query)) {
                        $db->query("update game_buylist set  `status`='0',pri_money='0',prizenum='0',prize_time='',pri_number='{$number}',isprize='' where id='{$row['id']}'");
                        if ($row['pri_money'] > 0) {
                            $game_info = $row;
                            ...
                            $hig_money = -$pri_money;
                            $backmony = $hig_money;
                            $db->query("update user_bank set hig_amount=hig_amount+{$hig_money} where userid='{$userid}' ");
                            $db->query("update user_bank set low_amount=low_amount+{$low_money} where userid='{$userid}' ");
