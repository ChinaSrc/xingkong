...
if($active=='ssc_list'){
	...
	if($id>0){
		...
		mysql_query("insert into game_ssc_list(`status`) values('0')");
	...
	if($id>0){
		...
		foreach ($_POST as $key=> $value) {
			mysql_query("update `game_ssc_list` set `{$key}`='{$value}' where id='{$id}'");
		}
	}
...
if($active=='ssc_prize'){
		...
		foreach ($_POST['prize'] as $key=> $value) {
			mysql_query("update `game_ssc_list` set `prize`='{$value}' where id='{$key}'");
		}
		add_adminlog( "更新时时彩玩法奖金");
...
foreach ($modes as $value) {
	...
	$list1=$db->fetch_all("select * from game_code where pid in (select id from  game_code where ckey='{$value}') order by id asc");
	if($list1){
...
echo "<div style='font-size:14px;text-align:center;background:#FFFFFF;'><font style='font-color:#ffffff;'>“玩法选择”中所勾上的玩法应该包念含“默认玩法”中所选择的玩法</font></div>";
echo "<script>setTimeout(\"history.back()\",3000)</script>";
}
$_POST['first_cate']=$first_cate;
if($fullname==""or $ckey==""){
echo "<div style='font-size:14px;text-align:center;background:#FFFFFF;'><font style='font-color:#ffffff;'>所有内容必填</font></div>";
...
if($id==""){
mysql_query("set names utf8;");
$strSql="insert into game_type(fullname,ckey,lot_date,lot_num,lot_begin,lot_end,code,status,`ico`) value ('$fullname','$ckey','$lot_date','$lot_num','$lot_begin','$lot_end','$code','0','$ico')";
mysql_query($strSql) or die("插入时出错2".mysql_error());
$id=mysql_insert_id();
add_adminlog( "添加新游戏:".$fullname);
}else{
mysql_query("set names utf8;");
if($ico) $str=" ,`ico`='{$ico}'";
$strSql="update game_type set fullname='$fullname',ckey='$ckey',lot_date='$lot_date',lot_num='$lot_num',lot_begin='$lot_begin',lot_end='$lot_end',firstcode='$first_cate',code='$modeslist',status='$status' {$str} where id='$id'";
mysql_query($strSql) or die("插入时出错1".mysql_error());
add_adminlog( "修改游戏信息:".$fullname);
}
if($id>0){
	
	foreach ($_POST as $key=> $value) {
		mysql_query("update game_type set `$key`='{$value}' where id='{$id}'");
	}
	
}

Arr_File::ArrGames();Arr_File::ArrCodes();
echo "<div style='font-size:14px;text-align:center;background:#FFFFFF;'><font style='font-color:#ffffff;'>保存成功</font></div>";
echo "<script>setTimeout(\"parent.window.location.reload()\",1000)</script>";
echo "<script>setTimeout(\"parent.pop.close()\",1000)</script>";
}
...
mysql_query($strSql,$link) or die("插入时出错66".mysql_error());
add_adminlog( "修改开奖时间");
}
}else{
for($i=0;$i<count($begintime);$i++){
...
if($id==""){
mysql_query("set names utf8;");
$strSql="insert into game_code(fullname,ckey,cate,pid,status) value ('$fullname','$ckey','$cate','$pid','$status')";
mysql_query($strSql,$link) or die("插入时出错2".mysql_error());
add_adminlog("添加新玩法：{$fullname}");
}else{
mysql_query("set names utf8;");
$strSql="update game_code set fullname='$fullname',ckey='$ckey',cate='$cate',pid='$pid',status='$status' where id='$id'";
$db->query($strSql);
...
if($active=='admin_pwd'){
	...
	echo "<script>alert('原始登录密码输入不正确');window.history.go(-1); </script>";
		...
		$pwd=md5($_POST['ps1']);
	mysql_query("update user  set `password`='{$pwd}' where userid='{$_SESSION['admin_id']}'")	;
	add_adminlog("修改登陆密码");
...
if($id>0){
	...
	foreach ($_POST  as $key =>$value) {
		...
		mysql_query("update `user` set `$key`='{$value}' where userid='{$id}'");
	}
	add_adminlog( "修改管理员{$_POST['username']}信息");
}
	else{
		$username=$_POST['username'];
	$query=mysql_query("select * from user where username='$username' and admin='1'");
...
exit();
		...
		mysql_query("insert into user(`admin`,`registertime`) values('1',now())");
		
		if(mysql_affected_rows()>0){
			
			$id=mysql_insert_id();
			
			$_POST[randcode]=rand(100000, 999999);
			foreach ($_POST  as $key =>$value) {
		if($value or $key!='password')
		mysql_query("update `user` set `$key`='{$value}' where userid='{$id}'");
	}
...
if($bank_pass!=""){$bank=" password=md5($bank_pass)";}else{$bank="";}
...
exit();		
		...
		if($_POST['fenhong']>$pre['fenhong']){
echo "<div style='font-size:14px;text-align:center;background:#FFFFFF;'><font style='color:#ff0000;'>分红不能大于上级代理的粉红【{$pre['fenhong']}%】</font></div>";
echo "<script>setTimeout(\"history.back()\",1000)</script>";
exit();		
}
...
$modes=1700+20*$rebate;

if($id>0){
$strSql="update user set sex='$sex',rebate='$rebate',status='$_POST[status]',modes='$modes',higherid='{$higherid}',isproxy='$isproxy',proxynum='$proxynum',mobilephone='$mobilephone',qqnum='$qqnum',mark='$_POST[mark]',fenhong='$_POST[fenhong]',ps_pre1='$_POST[ps_pre1]',ps_pre2='$_POST[ps_pre2]' $pass where userid='$id'";
...
exit();
	...
	$strSql="insert into user(username,password,modes,higherid,role,sex,isproxy,proxynum,mobilephone,qqnum,`status`,registertime,rebate,fenhong,mark,ps_pre1,ps_pre2) values('$username','".md5($password)."','{$modes}','$higherid','$role','$sex','$isproxy','$proxynum','$mobilephone','$qqnum','$_POST[status]',now(),'$rebate','{$_POST[fenhong]}','$_POST[mark]' ,'$_POST[ps_pre1]','$_POST[ps_pre2]')";
...
if(!$id){
$id=$uid=mysql_insert_id();
if($uid>0)	{
	mysql_query("insert into user_bank(userid,password,status) values('$uid','".md5($bank_pass)."','{$_POST['bank']['status']}' )");	
...
mysql_query($strSql);
	...
	$strSql="update user_bank set status='{$_POST['bank']['status']}' where userid='$id'";

mysql_query($strSql);
if($_POST[user_bank]){
		if(!$db->fetch_first("select * from user_bank_list where userid='{$id}'"));
		$db->query("insert into user_bank_list(userid,is_first) values('{$id}','yes')");	
foreach ($_POST[user_bank] as $key=>$value){
	
		$strSql="update user_bank_list set `$key`='{$value}'  where userid='$id'";
...
mysql_query("delete from user_mibao where userid='{$id}'");
if($_POST[selectone])
mysql_query("insert into user_mibao (userid,question,answer,time) values('$id','$_POST[selectone]','$_POST[answer1]','{$now}')");
if($_POST[selecttwo])
mysql_query("insert into user_mibao (userid,question,answer,time) values('$id','$_POST[selecttwo]','$_POST[answer2]','{$now}')");
if($_POST[selectthree])
mysql_query("insert into user_mibao (userid,question,answer,time) values('$id','$_POST[selectthree]','$_POST[answer3]','{$now}')");
	

echo "<div style='font-size:14px;text-align:center;background:#FFFFFF;'><font style='font-color:#ffffff;'>提交成功</font></div>";
echo "<script>setTimeout(\"window.location='{$_POST['backurl']}'\",1000)</script>";
...
$headerHTML.=" <td width='20%'>玩法名称</td><td width='5%'>排序</td><td width='10%'>返点计算</td><td width='10%'>最大注数</td><td width='20%'>操作</td></tr>";
...
$bodyHTML.="<td align=center><select id='Rebate_".$uid."'>";
...
$bodyHTML.="</select></td>";
$bodyHTML.="<td align=center><input id='MaxNote_".$uid."' value='".$rowslogs[MaxNote]."' size=3></td>";
...
if($table=='user_fenhong_log' or $table=='user_fandian_log'){
	$key='time';
...
$results_per=mysql_query($sql_per);
...
echo "<script>setTimeout(\"window.location='".$_SERVER['HTTP_REFERER']."'\",1000)</script>";
	...
	if($str == true){
		if($_POST['id']){
		  ...
		  $query=	mysql_query("select * from game_buylist where playkey='{$_POST['playKey']}' and period='{$_POST['period']}' order by id asc");
		  	while ($row=mysql_fetch_array($query)){
		  		if($row['pri_money']>0){
		  			...
		  			mysql_query("delete from user_bank_log where floatid='{$row['id']}' and cate='hig_prize'");

		  		}
		  		
		  		
		  		mysql_query("update game_buylist set status='0',pri_money='',isprize='',prizenum='0' where id='{$row['id']}'");
		  		
		  	}
         
		  	
		  	mysql_query("update game_lottery set status='0' where id='{$_POST['id']}'");
        		add_adminlog("重新派奖");
        }
