...
function TaskMenu(strLabel,menuState)
{
	...
	this.setBackground = function(srcImage)
	{
		...
		{
			if(srcImage != null){
				this.menuBody.style.background = "url(" + srcImage + ")  0px  0px";
	...
	{	
		...
		if(tfDistance > 0)
			//TaskMenu.collection[" + this.menuID + "]._extend()
			this.invalidateTimer = setTimeout("TaskMenu.collection[" + this.menuID + "]._extend()",200);
		
		else
			//TaskMenu.collection[" + this.menuID + "]._extend()
			this.invalidateTimer = setTimeout("TaskMenu.collection[" + this.menuID + "]._shorten()",200);
	}
	this.click = function()
	{
...
JS.addListener(window,"resize",new Function("setTimeout(\"TaskMenu._refreshAll()\",10);"),false);

JS.addListener(window,"resize",new Function("setTimeout(\"TaskMenu._onClick()\",10);"),false);
