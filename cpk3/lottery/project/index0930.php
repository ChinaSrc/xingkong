...
$result3=mysql_query($sql3);$listnum=0;
$nums3=mysql_num_rows($result3);
$result9 = mysql_query("select count(game_buylist.id) from $db_s where $search") or die("未能读取，请刷新");
$rows9=mysql_fetch_row($result9);	
...
$rows3=$db->exec("select game_buylist.*,user.username from game_buylist,user where  user.userid=game_buylist.userid  and (game_buylist.z_number='{$rows3['z_number']}' or game_buylist.id='{$rows3['z_number']}')   order by game_buylist.period asc");
...
echo "<td><a   onclick=\"javascript:winPop({title:'修改用户信息',form:'Form1',width:'600',height:'550',url:'?controller=user&action=index_add&id={$rows3['userid']}'});\" >{$rows3[username]}</a></td>";
