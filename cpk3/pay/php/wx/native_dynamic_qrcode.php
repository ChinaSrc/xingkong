...
<head>
    ...
    <script language="javascript" type="text/javascript" src="/wxres/JQuery132.js"></script>
	...
	/*
    var a = function() {
        $.getJSON("http://account.1yyg.com/JPData/API.ashx?action=getwxpayresult&id=" + d + "&fun=?",
        function(f) {
            if (f.state == 0) {
                if ($("#hidIsBuyPay").val() == "0") {
                    location.replace("http://member.1yyg.com/userbalance.html")
                } else {
                    location.replace("http://www.27yg.com/mycart/shopok.html?id=" + d)
                }
            } else {
                b++;
...
<body>
    ...
    <div class="wx_header">
        <div class="wx_logo"><img title="一元闪购微信支付" alt="微信支付标志" src="/wxres/wxlogo_pay.png" /></div>
    </div>
    <div class="weixin">
        <div class="weixin2">
	...
	<script src="/wxres/qrcode.js"></script>
	<script>
				...
				success: function(res){
				    if(res.code!=999){
						if(res.code==4){
						   ...
						   setTimeout(function(){alert('支付成功');location.href='http://www.zongyi99.com';},3000);
