...
<div class="container">
		...
		$pMerCode = '184255';//商户号
		$pMerName = 'pay';//商户名
		$pMerCert = 'PYS525vN53ox2xvQTk9UXzuFuFqarAhKApl0ZJrrcG4QLSA9Kjx1f1oKsctJHOSs8YkCVJ9BztAGJ9Efsp4AJOmTMyvCEZozYdb9STXy53En7RVX79hqNdeR5zN62FAL';//商户证书
		$pAccount  = '1842550012';//账户号
		$pMsgId = "msg".rand(1000,9999);;//消息编号
		$pReqDate = date("Ymdhis");//商户请求时间

		
		$pMerBillNo = date("YmdHis",time())."_".$_REQUEST['orderid'];//商户订单号
		$pGoodsName = "recharge";//商品名称
		$pGoodsCount = "";
		$pOrdAmt = $_REQUEST['amt'];//订单金额 
		$pOrdTime =date("Y-m-d H:i:s");
	
		$pMerchantUrl='http://'.$_SERVER['HTTP_HOST'].'/wx/success.php';
		$pServerUrl='http://'.$_SERVER['HTTP_HOST'].'/wx/result.php';
		$pBillEXP="";
		$pReachBy="";
		$pReachAddress="";
		$pCurrencyType="156";
