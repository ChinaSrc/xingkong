...
function styleAlert(message) {
  ...
  $(window.top.document).find("body").append($alert);
  $(window.top.document).find(".closeTip, .closeBtn").click(function() {
    $alert.remove();
  })
}
// confirm
function styleConfirm(message, confirmCB) {
  var height = $(window.top.document).height();
  var $alert = $(
    '<div class="j-ui-miniwindow pop w-9" style="z-index: 700; position: fixed; display: block; left: 0; top:0; bottom:0; right:0; margin:auto; height:150px;"><div class="hd"><i class="close closeBtn"></i><span class="title">温馨提示</span></div><div class="bd"><div class="bd text-center"><div class="pop-waring"><i class="ico-waring"></i><h4 class="pop-text" style="max-width:310px"><span>' +
    message +
    '</span><br></h4></div></div></div><a id="del" href="javascript:void(0);" style="display: inline-block;" class="btn closeTip">确 认<b class="btn-inner"></b></a><a id="cancel" href="javascript:void(0);" style="display: inline-block;" class="btn closeTip">取 消<b class="btn-inner"></b></a><a href="javascript:void(0);" style="display: none;" class="btn closeTip">关 闭<b class="btn-inner"></b></a></div><div class="j-ui-mask" style="position: absolute; left: 0px; top: 0px; opacity: 0.5; z-index: 600; width: 100%; height: '+height+'px; display: block; background-color: rgb(51, 51, 51);"></div>'
  );
  $(window.top.document).find("body").append($alert);
  $(window.top.document).find(".closeTip, .closeBtn").click(function() {$alert.remove();})
  $(window.top.document).find("#del").click(function(){confirmCB();})
  $(window.top.document).find("#cancel").click(function(){$alert.remove();})
}

var sAlert = styleAlert;
var sConfirm = styleConfirm;

// 下面兩個好像用不到...要用的話要改樣式+引入遊戲框架....
function nAlert(message) {
    if ( typeof phoenix !== 'undefined' ) {
        var msg = phoenix.GameMessage.getInstance();
        msg.show({
            content: message,
            iconIsShow: true,
            closeIsShow: true,
        ...
        window.console && console.log('lack of phoenix object');
    }
}

function nConfirm(message, confirmCB, cancelCB) {
    if ( typeof phoenix !== 'undefined' ) {
        var msg = phoenix.GameMessage.getInstance();
        msg.show({
            content: message,
...
//密码验证(8－16位数字和字母，不能只是数字，或者只是字母，不能连续三位相同)
function validateUserPss( str )
{
	var patrn = /^[0-9a-zA-Z]{8,16}$/;
	if( !patrn.exec(str) ){
		return false;
	}
	patrn = /^\d+$/;
...
function srip2tInit() {
	...
	jQuery("#starttime").change(function(){
		if(! validateInputDate(jQuery("#starttime").val()) )
		{
			...
			sAlert("时间格式不正确,正确的格式为:2009-06-10 10:59");
		}
		if($("#endtime").val()!="")
		{
...
if(tag!=temp) siderbarInit(obj,trl,tag);
	...
	$("#contentBox").each(function(){
		var spanTemp = $(this);
		spanTemp.html('<div align=center><img src="images/sy/loadingAnimation.gif"><br><br>正在载入......</div>');
		$.ajax({
				type:"POST",
				//url: "mainContent.html",
				url: trl+"_"+obj+".shtml@ajax=1"+tagurl,
				timeout:30000,
				dataType: "html",
...
function  nochecksercuity(checkid){
	...
	$("#contentBox").each(function(){
		...
		$.ajax({
				type:"POST",
				//url: "mainContent.html",
				url: "security_platwithdraw.shtml@check="+checkid,
				timeout:30000,
				dataType: "html",
				success:function(data)
				{
        ...
        $.ajax({
            ...
            success : function(data){
                if( data == "error" ){//
                    ...
                    var partn = /<(.*)>.*<\/\1>/;
                    if( partn.exec(data) ){
                        window.top.location.href="default_main.shtml";
                        return false;
                    }
                    $("#checkmoneys").html(moneyFormat(data));
                    return true;
                }
            },
            error: function(){
