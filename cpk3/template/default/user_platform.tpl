...
<!--{include file="<!--{$tplpath}-->head.tpl"}--> 
    ...
    <link href="<!--{$file_uri}-->/<!--{$skinpath}-->2017/Css/mycenter.css" type="text/css" rel="stylesheet" />
                                ...
                                <tr >
                                    <td  align="center">
                                        ...
                                        状态 <select  name='status'  onchange="document.getElementById('form1').submit();">
                                            ...
                                            <option value="0" <!--{if $smarty.get.status eq '0'}-->selected<!--{/if}-->>审核中</option>
                                            <option value="1" <!--{if $smarty.get.status eq '1'}-->selected<!--{/if}-->>提现成功</option>
                                            <option value="2" <!--{if $smarty.get.status eq '2'}-->selected<!--{/if}-->>提现失败</option>
                                        </select>
      ...
      <a  class="userSearch "  href="home_user_recharge.html?isgetdata=yes&st=<!--{$smarty.get.st}-->&begindate=<!--{$begindate}-->&enddate=<!--{$enddate}-->&username=<!--{$smarty.get.username}-->">充值记录</a>
       <a  class="userSearch active" >提现记录</a></span>
