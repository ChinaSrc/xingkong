...
<div style='background-color:#fff;'>
    <form name="form" action="home_safe_msginfo.html?id=<!--{$smarty.get.id}-->&from=parent" method="post">
                ...
                <!--{foreach from=$list key=key item=item}-->
                            <li class="mar-btm">
                                <div class="media-<!--{$item['css']}-->">
                                    <img src="<!--{if $item.fromname eq '管理员'}--><!--{$file_uri}-->static/images/av<!--{if $item['css']=='left'}-->4<!--{else}-->1<!--{/if}-->.png<!--{else}--><!--{avatar($item['perid'])}--><!--{/if}-->" class="img-circle img-sm" alt="Profile Picture">
                                </div>
