...
<!--{include file="<!--{$tplpath}-->user_top.tpl"}-->
	 ...
	 <script>selectSetItem(G('is_prize'),'<!--{$is_prize}-->')</script>
                                   ...
                                   &nbsp;日期：
                                    <input type="text" name="begindate" id="begindate" value="<!--{$begindate}-->"  class="Wdate" style='width:100px;'type="text" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',alwaysUseStartDate:false})"/>
	&nbsp;至

	 <input type="text" name="enddate" id="enddate" value="<!--{$enddate}-->"  class="Wdate" type="text" style='width:100px;' onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',alwaysUseStartDate:false})" />&nbsp;
