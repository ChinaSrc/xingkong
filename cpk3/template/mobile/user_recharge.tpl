...
<div  id="leftTabBox">
    <div  class="hd"><ul >
            ...
            <li  class="userSearch active">充值记录</li>
            <li  class="userSearch  " onclick="location.href='home_user_platform.html?isgetdata=yes&st=<!--{$smarty.get.st}-->&begindate=<!--{$begindate}-->&enddate=<!--{$enddate}-->&username=<!--{$smarty.get.username}-->';">提现记录</li>
...
&nbsp;&nbsp;至
	 <input type="text" name="enddate" id="enddate" value="<!--{$enddate}-->" class="Wdate" style="width:100px;"   onclick="WdatePicker({dateFmt:'yyyy-MM-dd',alwaysUseStartDate:false})">&nbsp;
      ...
      <input style="width: 120px;<!--{if $smarty.get.st<2 || !$smarty.get.username}-->display: none<!--{/if}-->" class="textbox" name="username" type="text" id="username" value="<!--{$smarty.get.username}-->" size="20" />
    ...
    <input type="submit" value="搜 索" class="button">
      ...
      <div class='wap_list' >
          ...
          <div>
              ...
              <span style="float: right;">        <!--{if $value['status']=='0'}-->等待汇款<!--{/if}-->
                  <!--{if $value['status']=='1'}-->充值成功<!--{/if}-->
                  <!--{if $value['status']=='2'}-->充值失败<!--{/if}-->    </span>
